...
class AuditTrail(object):
    def __init__(self, show_in_admin=False, save_change_type=True, audit_deletes=True,
                 track_fields=None, check_audit=None):
        self.opts = {}
        ...
        self.opts['check_audit'] = check_audit
        ...
        def _contribute(sender, **kwargs):
            ...
            def _audit_track(instance, field_arr, **kwargs):
                ...
                try:
                    ...
                    if len(field_arr) > 2:
                        if isinstance(field_arr[2], collections.Callable):
                            fn = field_arr[2]
                            return fn(instance)
                        else:
                            return field_arr[2]
            ...
            def _audit(sender, instance, created, **kwargs):
                ...
                if self.opts['check_audit']:
                    if not self.opts['check_audit'](instance):
                        return
                kwargs = {}
