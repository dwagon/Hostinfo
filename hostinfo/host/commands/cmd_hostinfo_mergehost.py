...
class Command(HostinfoCommand):
    ...
    def parseArgs(self, parser):
        parser.add_argument('-f','--force',help="Force the merge", action='store_true')
        parser.add_argument('-k','--kidding',help="Don't actually make any changes", action='store_true')
    ...
    def transferSingleKey(self, srckey, srchostobj, dsthostobj):
        ...
        if dstkey.value!=srckey.value:
            ...
            if not kidding:
                srckey.delete(readonlychange=True)	# Data is the same so delete the src
        return True
